...
class GLSLTemplate {
  ...
  setTexturesRatio() {
    ...
    for (let i = 0; i < this.textures.length; i++) {
      ...
      const textureRatio =
        t.texture.image.naturalWidth / t.texture.image.naturalHeight;
      t.ratio =
        this.winRatio > textureRatio
          ? new THREE.Vector2(1.0, textureRatio / this.winRatio)
          : new THREE.Vector2(this.winRatio / textureRatio, 1.0);
    }
  }
...
const powderImages = ["powder-1.jpg", "powder-2.jpg", "powder-3.png"];
// descriptions
const powderDesc = `Country Cross provides freshly created moringa powders and capsules, beauty products made from the leaves of Moringa Oleifera. Moringa offers a bounty of vitamins and nutrients as well as calcium, iron, magnesium, potassium and zinc, all helpful for your body. 
Moringa products are good for those suffering from malnutrition due to the high protein and fibre content. It’s also been known to prevent, treat and manage chronic illnesses such as anaemia, diabetes, obesity, cardiovascular and inflammatory diseases. 
You can always opt for the package product that comes to you in the form of powders and capsules. `;
...
const powderBtn = document.querySelector(".powder");
...
powderBtn.addEventListener("click", () => {
  changeImage(powderImages);
  product_description_change(powderDesc);
});
...
function changeImage(imageArr) {
  ...
  document
    ...
    .removeChild(document.querySelector("#music-container audio"));
  animationCarousel(240);
}
